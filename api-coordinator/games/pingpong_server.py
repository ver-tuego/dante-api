...
def create_server(port):
    ...
    server.bind(('localhost', port))  # Замените на порт, полученный от координатора
    server.listen(2)
    ...
    actionDB.update_queue(player_id_1, port, 1)
    print(f"Player id:{player_id_1} connected from", addr1)
    p1.sendall(b'1')  # Отправляем игроку номер 1

    p2, addr2 = server.accept()
    player_id_2 = p2.recv(1024).decode()  # Получаем id игрока 2
    actionDB.update_queue(player_id_2, port, 2)
    print(f"Player id:{player_id_2} connected from", addr2)
    ...
    while True:
        ...
        if paddle1.rect.colliderect(ball.rect) or paddle2.rect.colliderect(ball.rect):
            ball.speed[0] = -ball.speed[0] * 1.1  # Увеличиваем скорость на 10% при каждом отскоке
    ...
    actionDB.add_stat_pingpong(int(winner), 1)
    actionDB.add_stat_pingpong(int(loser), 2)
